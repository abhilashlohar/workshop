...
<link href="<?php echo $webroot_path ?>assets/global/plugins/fancybox/source/jquery.fancybox.css" rel="stylesheet" type="text/css"/>
<link href="<?php echo $webroot_path ?>assets/admin/pages/css/portfolio.css" rel="stylesheet" type="text/css"/>
...
<script type="text/javascript" src="<?php echo $webroot_path ?>assets/global/plugins/jquery-mixitup/jquery.mixitup.min.js"></script>
<script type="text/javascript" src="<?php echo $webroot_path ?>assets/global/plugins/fancybox/source/jquery.fancybox.pack.js"></script>
...
<script src="<?php echo $webroot_path ?>assets/admin/pages/scripts/portfolio.js"></script>
<script>
...
Demo.init(); // init demo features
   Portfolio.init();
   $(".liker").live("click",function(e){
		e.preventDefault();
		var s_id=$(this).attr("s_id");
		$(this).removeClass("liker");
		$("a[s_id="+s_id+"]").addClass("btn-primary");
		$("a[s_id="+s_id+"]").find("i").css("color","#FFF");
		$("div[s_id="+s_id+"]").css("visibility","visible");
		var numbr=$("div[s_id="+s_id+"]").find("span.numbr").text();
		numbr++;
		$("div[s_id="+s_id+"]").find("span.numbr").text(numbr);
		$.ajax({
		   url: "<?php echo $webroot_path ; ?>i/submit_like/"+s_id,
		   success: function(data){
			   
		   }
		 });
	})
	$(".send_video").live("click",function(e){
		e.preventDefault();
		var v_id=$(this).attr("v_id");
		$.ajax({
		   url: "<?php echo $webroot_path ; ?>i/send_video/"+v_id,
		   success: function(data){
				$("button[v_id="+v_id+"]").hide();
		   }
		 });
	})
});
